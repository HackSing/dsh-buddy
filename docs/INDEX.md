...
- [Release CI:tag 触发双平台构建并发布 GitHub Release](plans/release-ci.md) — 状态：有效（实施中）；关键符号：`release.yml`、`dist:win`、`GITHUB_REF_NAME`、`gh release upload`
...
- [Release CI 验收](acceptance/release-ci.md) — 状态：有效（待验收）；关键符号：`release.yml`、`dist:win`、`gh release upload`
