...
- [dispatch vendored sqlite 多平台化,解除 Windows 挂账](plans/multiplatform-sqlite-vendor.md) — 状态：有效（实施中）；关键符号：`seedNativeAddons`、`patchDatabaseJs`、`platformDirs`、`PREBUILD_TARGETS`
...
- [dispatch 多平台 sqlite vendor 验收](acceptance/multiplatform-sqlite-vendor.md) — 状态：有效（待验收）；关键符号：`seedNativeAddons`、`patchDatabaseJs`、`platformDirs`
