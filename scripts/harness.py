...
- **完整不等于扩大范围**：同一交付物缺的部分、验证、依赖说明、文档同步自己补上；重构没让改的模块、改没提过的配置、删文件、提交代码等范围扩张先问用户。修改面约束见"不顺手加固"。
...
先列需要用户决定或确认的事项（含所做的假设、偏离用户要求之处），没有就写"无"；再报告实际改动路径、执行命令与退出结果、验收层、未覆盖项和剩余风险，发现多个问题时按严重性排序。失败、跳过、未完成的部分单独写明，不藏在成功的叙述里。没有证据时不得声称完成。"""
