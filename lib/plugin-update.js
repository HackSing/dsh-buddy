...
async function downloadTarball({ url, sha256, downloadDir, fetchImpl }) {
  ...
  try {
    await pipeline(Readable.fromWeb(res.body), fs.createWriteStream(staging));
...
async function applyPluginUpdate({ update, dshHome, profileName, downloadDir, fetchImpl = fetch }) {
  let tarballPath;
  try {
    tarballPath = await downloadTarball({ ...update.tarball, downloadDir, fetchImpl });
  } catch (err) {
