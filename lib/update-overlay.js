...
const I18N = {
  ...
  en: {
    app: 'Downloading update',
    plugin: 'Downloading plugin update',
...
// progress 可能是 electron-updater 的 download-progress 载荷
// ({ percent, transferred, total, bytesPerSecond }),也可能是插件 tar 下载的
// 简化载荷({ transferred, total }):percent 缺省时由 transferred/total 推导。
// 字段缺失时降级,不抛错。subject: 'app'(应用整包)| 'plugin'(插件热更)。
function overlayViewModel({ stage, locale = 'zh', subject = 'app', progress, errorMessage }) {
  const strings = I18N[locale] || I18N.en;
  if (stage === OVERLAY_STAGE.downloading) {
    let percent = progress && Number.isFinite(progress.percent) ? progress.percent : null;
    if (
      percent === null &&
      progress &&
      ...
      percent = (progress.transferred / progress.total) * 100;
    }
    ...
    return {
      stage,
      title: `${strings[subject] || strings.app} ${Math.round(percent)}%`,
      detail: parts.join(' · '),
      percent,
      canRetry: false,
...
// ctx = { win, locale };返回控制器 { showDownloading, setProgress, reportError, hide }。
// 重试目标随下载主体走:showDownloading({ subject, onRetry }) 登记本次的重试动作
// (应用更新→检查更新;插件热更→检查插件更新),error 态点击重试时调用。
// reportError 在浮层不可见时静默略过:检查阶段的失败(404/离线)已有各自的
// 用户反馈(手动检查弹窗/自动检查日志),浮层只接管「下载已开始但失败」这段盲区。
function createUpdateOverlay({ win, locale = 'zh' }) {
  ...
  const apply = (next) => {
    model = next;
    if (next.stage === OVERLAY_STAGE.hidden) retryHandler = null; // 收场即清,不跨次复用
  ...
  ipcMain.on('update-overlay:retry', (event) => {
    ...
    if (model.stage !== OVERLAY_STAGE.error || !retryHandler) return; // 仅 error 态允许重试
    retryHandler();
  });
  ...
  return {
    showDownloading({ subject: nextSubject = 'app', onRetry } = {}) {
      subject = nextSubject;
      retryHandler = onRetry || null;
      apply(overlayViewModel({ stage: OVERLAY_STAGE.downloading, locale, subject, progress: null }));
    },
    setProgress(progress) {
      if (model.stage !== OVERLAY_STAGE.downloading) return; // 只在下载态推进度
      apply(overlayViewModel({ stage: OVERLAY_STAGE.downloading, locale, subject, progress }));
    },
    reportError(message) {
