...
// 发布侧脚本(scripts/build-plugin-channel.js)把更新 tar 传到同一滚动 release 下,
// 客户端实际下载地址以 channel JSON 里的 tarball.url 为准,此处只是单一真源。
const CHANNEL_TARBALL_URL =
  'https://github.com/HackSing/dsh-buddy/releases/download/plugin-channel/web-profile-plugins.tar.gz';
...
// update 存在时携带 { fingerprint, packages, updates, tarball, minDshVersion, installable };
// installable=false 表示 channel 要求的 dsh 本体比内嵌的新,只提示不安装
// (插件 keyed slot 等兼容门槛,见 plugins/preinstall-manifest.json comment)。
...
async function checkPluginChannel({
  ...
  const channel = parseChannel(channelBody.body);
  if (!channel) {
    writeState(stateDir, nextState);
