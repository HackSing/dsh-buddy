...
// 本地缺失或版本不可解析一律视为需要更新——无法证明足够新,
// 与 profileUpgradeDecision 的保守方向一致。
// 例外(决策 B):本地 spec 以 file: 开头的包跳过——那是开发者手工本地安装,
// 提示更新会反复骚扰且安装会回滚开发覆盖。
function diffChannelVersions(packages, localDeps) {
  const updates = [];
  for (const p of packages) {
    const local = localDeps[p.name];
    if (typeof local === 'string' && local.startsWith('file:')) continue;
...
// rc/preview 阶段上游把候选发在 next 上(0.1.1-rc.1 挂 next 标签,latest 仍指 0.1.0-rc.7),
