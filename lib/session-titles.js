...
// host API unary 调用的端点契约(dsh 0.1.5-rc.1 形状,三处一起变了,改一处就要全改):
// 路径即方法且方法名是两段斜杠(旧的点号形状 rc.5 回 404),payload 必须是单字段
// args 包住按参数名索引的实参,且 /api 只认授权 cookie(见 lib/dsh-browser-auth.js)。
const SESSION_LIST_METHOD = 'session/list';
const SESSION_LIST_PAYLOAD = { args: { _request: {} } };

// POST 一次 session/list(unary:信封 client-request,业务错误走 200 + result.ok=false)。
// cookie 缺省时请求必然 401,由调用方决定要不要在没有 cookie 时干脆别发(见 main.js)。
function postSessionList(url, { cookie, timeoutMs = REQUEST_TIMEOUT_MS } = {}) {
  return new Promise((resolve, reject) => {
      ...
      (res) => {
        ...
        res.on('end', () => {
          if (res.statusCode !== 200) {
            reject(new Error(`${SESSION_LIST_METHOD} HTTP ${res.statusCode}`));
            return;
          }
          try {
            const message = JSON.parse(Buffer.concat(chunks).toString('utf8'));
            if (message?.result?.ok !== true) {
              reject(new Error(`${SESSION_LIST_METHOD} rejected: ${JSON.stringify(message?.result?.error ?? message)}`));
              return;
            }
    ...
    req.on('error', reject);
    req.setTimeout(timeoutMs, () => req.destroy(new Error(`${SESSION_LIST_METHOD} request timed out`)));
    req.end(body);
  });
}
...
// 仅作诊断,不向上抛。
async function waitForTitlesSettled(url, { timeoutMs = DEFAULT_TIMEOUT_MS, intervalMs = DEFAULT_INTERVAL_MS, cookie, listFn } = {}) {
  const list = listFn ?? (() => postSessionList(url, { cookie }));
