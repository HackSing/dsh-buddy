...
// - 已存在且 dependencies 与清单一致 → 'up-to-date',不动磁盘;
...
function profileUpgradeDecision(deps, manifestPackages) {
  ...
  const stale = manifestPackages.some((p) => deps[p.name] !== p.version);
...
module.exports = { installBundledProfile, profileUpgradeDecision };
