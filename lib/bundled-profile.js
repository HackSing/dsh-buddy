...
const { patchProfileUi } = require('./patch-multimodal-ui');
...
// 给已安装（或刚解包）的 web profile 打上 llm-pi-ai 多模态 UI 补丁。
// 幂等：已经打过补丁的 client.js 会原样跳过；profile 里没有该 UI 包时返回 false。
function patchBundledProfileUi({ dshHome, profileName }) {
  return patchProfileUi({ dshHome, profileName });
}

module.exports = { installBundledProfile, patchBundledProfileUi };
