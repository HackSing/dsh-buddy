...
// - 已存在且含清单外依赖:
//   清单内存在真实待升级(落后/缺包/含退休包) → 'preserved',原样保留并给出包名(弹窗);
//   清单内全部满足 → 'preserved-current',原样保留并给出包名(仅日志,不弹窗);
// - package.json 不可读 → 'preserved'(无法判定,保守弹窗)。
...
//
// 解包走 tar 的 promise 模式而非 sync:实测随包 profile 有 1.4 万+文件、
// 解压近 18s,sync 模式会把 Electron 主进程事件循环整段阻塞,启动加载窗口
// 将无法呈现/假死;promise 模式逐条目让出事件循环,窗口保持响应。
// applyLinks 保持同步:链接条目数量小,单次成本毫秒级。
...
async function collectLinks(tarballPath) {
  const links = [];
  await tar.t({
    file: tarballPath,
    onReadEntry: (entry) => {
...
// deps 为 null(package.json 缺失/不可读)时一律 preserved——
// 无法证明替换安全,宁可保留也不覆盖用户环境。
// 比较带方向:profile 版本不低于清单(含运行时热更领先于随包清单的情形)即视为
// 满足,不做替换;只有落后、缺包或版本不可解析才判定 upgrade——否则热更到新版
// 的 profile 会被随包旧清单回滚。
// 例外(决策 B):profile 侧版本字符串以 file: 开头视为开发者本地覆盖,判满足——
// 若按「不可解析即落后」会让手工 file: 安装的同名包每次启动被整目录替换回滚。
function profileSatisfiesManifest(profileVersion, manifestVersion) {
  if (typeof profileVersion === 'string' && profileVersion.startsWith('file:')) return true;
...
// 四态判定:
// - 'up-to-date':清单内全部满足,无清单外依赖;
// - 'upgrade':清单内存在真实待升级(落后/缺包/不可解析),或 deps 含 retired
//   退休包(决策 C:退休包不计 extras、出现即强制整目录备份替换,替换后的新
//   profile 自然不含退休包,完成存量清退);
// - 'preserved':有清单外依赖且清单内存在真实待升级——真实更新被清单外插件挡住,
//   保留并报名(弹窗);
// - 'preserved-current':有清单外依赖但清单内全部满足——无真实更新,保留并报名
//   (仅日志,不弹窗)。
// retiredNames 为退休包名数组(来自 preinstall-manifest 的 retired 字段)。
function profileUpgradeDecision(deps, manifestPackages, retiredNames = []) {
  ...
  const extras = Object.keys(deps)
    .filter((name) => !manifest.has(name) && !retired.has(name))
    .sort();
  const hasRetired = Object.keys(deps).some((name) => retired.has(name));
  const stale =
    hasRetired ||
    manifestPackages.some((p) => !profileSatisfiesManifest(deps[p.name], p.version));
  if (extras.length > 0) return { status: stale ? 'preserved' : 'preserved-current', extras };
...
async function installBundledProfile({ tarballPath, dshHome, profileName, manifestPackages = [], retiredPackages = [] }) {
  ...
  if (!freshInstall) {
    ...
    const decision = profileUpgradeDecision(existingDeps, manifestPackages, retiredPackages);
    if (decision.status === 'up-to-date') return { status: 'up-to-date' };
    // preserved 与 preserved-current 一样原样返回不动磁盘;弹窗与否由调用方(壳)按状态区分
    if (decision.status === 'preserved' || decision.status === 'preserved-current') {
      return { status: decision.status, extras: decision.extras };
    }
  }
  ...
  try {
    const links = await collectLinks(tarballPath);
...
module.exports = { installBundledProfile, profileUpgradeDecision, readProfileDeps, backupDirName };
