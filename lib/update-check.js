...
// 发布页地址:帮助菜单「检查更新」等显式用户入口直接打开它,不走节流查询
const RELEASES_PAGE_URL = 'https://github.com/HackSing/dsh-buddy/releases';
...
// force=true 用于菜单「检查更新」等显式用户入口:绕过 24h 节流立即查询,
// 状态落盘语义不变(lastCheckedAt 照常刷新,自动检查的节奏不被手动检查打乱)。
async function checkForUpdate({ currentVersion, stateDir, notify, force = false, now = Date.now() }) {
  const state = readState(stateDir);
  if (!force && !isCheckDue(state, now)) return { outcome: UPDATE_OUTCOME.throttled };
