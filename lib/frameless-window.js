...
//   BaseWindow + 单个全高内容视图,窗口控制与拖拽约定由 dsh 页面插件经
//   buddy scheme 提供(桥注入 windowControls:true);曾经随包的实现
//   dsh-buddy-about 已移除,当前无随包插件挂载,该模式仅剩快捷键操作窗口。
// 三种模式共用:scheme 拦截 + setWindowOpenHandler、__DSH_BUDDY__ 注入、
// updateOverlay、reloadContent(本文件私有 helper),以及启动加载页
// (beginStartupLoading:先呈现 loading.html,dsh 就绪后经 win.loadContent 切换)。
...
const { LOADING_PAGE_PATH, buildStageScript } = require('./loading-page');
...
// 启动导航:先呈现本地加载页,dsh 就绪后由 main.js 经 win.loadContent 原地切换;
// 阶段文案经 win.setStartupStage 单向推送,注入失败只降级(与 attachBuddyInfo
// 的 warn-only 同一策略)。macOS 沉浸式分支(main.js)复用本函数,单一定义。
function beginStartupLoading(win, content) {
  win.loadContent = (url) => content.webContents.loadURL(url);
  // 内容视图的 WebContents 出口:页面守护(lib/page-guard.js)要监听其生命周期
  // 事件;borderless/legacy 的 content 是内部 WebContentsView,经此单一通道暴露。
  win.contentWebContents = content.webContents;
  // 任意时刻在内容页执行脚本(全局快捷键转发等壳→页面通知复用此入口)
  win.evalInContent = (code) => content.webContents.executeJavaScript(code);
  win.setStartupStage = (text) => {
    content.webContents.executeJavaScript(buildStageScript(text)).catch((err) => {
      console.warn(`[dsh-buddy] startup stage update failed: ${err.message}`);
    });
  };
  content.webContents.loadFile(LOADING_PAGE_PATH);
}
 ...
 */
function createNativeWindow({ version, onCheckUpdate, onCheckPluginUpdate }) {
 ...
 */
function createBorderlessWindow({ version, onCheckUpdate, onCheckPluginUpdate }) {
  const locale = resolveMenuLocale(process.env, os.homedir());
  ...
  beginStartupLoading(win, content);
  return win;
}

function createFramelessWindow({ version, onCheckUpdate, onCheckPluginUpdate }) {
