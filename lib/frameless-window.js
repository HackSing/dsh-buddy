...
const { RELEASES_PAGE_URL } = require('./update-check');
...
// ctx = { win, content, version };动作表数据化,新增菜单项只需在此处加一行。
const ACTIONS = {
  ...
  // 菜单点击是显式用户意图,不走 24h 节流的静默检查,直接打开发布页。
  'help:check-update': () => shell.openExternal(RELEASES_PAGE_URL),
  'help:about': (ctx) =>
...
function createFramelessWindow({ dshUrl, version }) {
