...
function createFramelessWindow({ dshUrl, version, onCheckUpdate, onCheckPluginUpdate, onRetryUpdate }) {
  ...
  // 插件热更重启 dsh 后由 main.js 调用,刷新内容视图指向新服务
  win.reloadContent = () => content.webContents.reload();
  // 更新下载进度浮层(第三个视图,叠在内容左下角):生命周期=下载生命周期,
  // error 态的重试点击转发给 main.js 注入的 onRetryUpdate(= 手动检查更新)。
  win.updateOverlay = createUpdateOverlay({
    win,
    locale: resolveMenuLocale(process.env, os.homedir()),
    onRetry: () => onRetryUpdate && onRetryUpdate(),
  });
  const sendState = () => {
